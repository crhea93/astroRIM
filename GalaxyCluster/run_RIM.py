...
synthetic data and rmfs. If yox_batch_testu change the internal structure, you will need to update `RIM_data_generator.py` to reflect the changes.
...
true_spectra_y = [data[1][1][min_:max_]/np.max(data[1][1][min_:max_])  for data in true_spectra_data]
...
valid_dataset = valid_dataset.prefetch(2)
...
plt.savefig('%s/test_convolved_%s.png'%(outputPath, name))
plt.clf()
...
plt.xticks(fontsize = 15)
plt.yticks(fontsize = 15)
plt.savefig('%s/test_true_%s.png'%(outputPath, name))
plt.clf()
...
pickle.dump(ysol_valid, open('%s/ysol_valid_%in_%ie_%its_%ib_%s.pkl' % (outputPath, nodes, epochs, t_steps, batch_size, name), 'wb'))
...
# Plot the prediction for the last sample
ysol_list = []
for val in ysol:
...
plt.savefig('%s/Test_Example_%in_%ie_%its_%ib_%.2Elr_%s.png' % (outputPath, nodes, epochs, t_steps, batch_size, learning_rate, name))
